...
export default function FormSelectFile(){
    ...
    const onClickButton =() =>{
        ...
        if(inputFile.current.files.length > maxNumFiles ){
            alert(`เลือกได้ไม่เกิน ${maxNumFiles}ไฟล`)
            return
        }
        ...
        <div className="mt-4 mx-auto p-3 rounded" style={{width:'400px',background:'#cee'}}>
            <form>
                ...
                <div className="form-group mb-3">
                    ...
                    <input type="file" id="file" accept="image/*" className="form-select form-control-sm" ref={inputFile} multiple/>
                </div>
                <div className="text-center mt-4">
                    <button type="button" className="btn btn-sm btn-primary px-4" onClick={onClickButton}>OK
                    </button>
                </div>
            </form>
        </div>
